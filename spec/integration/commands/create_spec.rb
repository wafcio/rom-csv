...
describe 'Commands / Create' do
  ...
  it 'returns a single tuple when result is set to :one' do
    result = users.try do
      users.create
        .call(id: 4, name: 'John', email: 'john@doe.org', title: 'Mr.')
    end

    expect(result.value)
      .to eql(id: 4, name: 'John', email: 'john@doe.org', title: 'Mr.')
  ...
  it 'returns tuples when result is set to :many' do
    result = users.try do
      users.create_many.call([
        { id: 4, name: 'Jane', email: 'jane@doe.org' },
        { id: 5, name: 'Jack', email: 'jack@doe.org' }
      ])
    end
